...
- CSRF must be in the **header** because Hostinger/PHP often drop DELETE/PATCH bodies (`_token` body alone caused production CSRF mismatch; dropped PATCH body made `is_internal` / `file_type` look required). Document **internal** and **type** toggles use `POST` + `_method=PATCH` and also send `is_internal` / `file_type` on the query string (controllers merge query into input). Agent/Hub active-inactive toggles use the same pattern with `status` on the query string.
